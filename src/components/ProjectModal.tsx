      ...
      {project && (
        ...
        >
          <div className="relative w-full max-w-4xl flex items-center justify-center">
            {/* Navigation Buttons */}
            <button onClick={handlePrevClick} className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/70 rounded-full p-2 text-slate-700 hover:bg-white transition-colors z-20 md:-left-16">
              <ArrowLeft size={28} />
            </button>
            <button onClick={handleNextClick} className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/70 rounded-full p-2 text-slate-700 hover:bg-white transition-colors z-20 md:-right-16">
              <ArrowRight size={28} />
            </button>

            <motion.div
              variants={modalVariants}
              className="bg-white rounded-lg shadow-xl w-full max-h-[90vh] flex flex-col md:flex-row relative overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Close Button */}
              <button onClick={onClose} className="absolute top-3 right-3 bg-slate-100/50 rounded-full p-1 text-slate-600 hover:text-slate-900 hover:bg-slate-200/70 transition-all z-20">
                <X size={20} />
              </button>

              {/* Image Preview */}
              <div className="w-full md:w-1/2 h-64 md:h-auto bg-slate-100 flex items-center justify-center relative flex-shrink-0">
                {!isImageLoaded && (
              ...
              <motion.div
                ...
                initial="hidden"
                animate="visible"
                className="p-6 md:p-8 flex flex-col overflow-y-auto"
              >
                <motion.h2 variants={textItemVariants} className="text-2xl md:text-3xl font-bold text-slate-800 mb-3">{project.title}</motion.h2>
